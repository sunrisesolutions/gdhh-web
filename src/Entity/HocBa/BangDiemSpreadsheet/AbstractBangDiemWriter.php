...
abstract class AbstractBangDiemWriter
{
    ...
    protected abstract function writeHeadingDauNam($namHocId, ChiDoan $chiDoan = null, HuynhTruong $truong = null);

    protected function writeBaseHeading($hocKy, $namHocId, ChiDoan $chiDoan = null, HuynhTruong $truong = null)
    {
        $sWriter = $this->sWriter;
        if ($hocKy === 'dau-nam') {
            $sWriter->writeCell("DANH SÁCH ĐẦU NĂM");
        } else {
...
//				'color' => array( 'rgb' => 'FF0000' ),
    ...
    public function writeDataDauNam(Collection $phanBoThieuNhi)
    {
        ...
        foreach ($phanBoThieuNhi as $phanBo) {
            ...
            $phanBoCu = $phanBo->getPhanBoTruoc();

            $thanhVien = $phanBo->getThanhVien();
            if (empty($phanBoCu)) {
                $bangDiem = null;
            } else {
                if (empty($bangDiem = $phanBoCu->getBangDiem())) {
                    $phanBoCu->setBangDiem($bangDiem = new BangDiem());
                }
            }
            $sWriter->writeCellAndGoRight('  '.$thanhVien->getId().'  ');
            $sWriter->writeCellAndGoRight('  '.$thanhVien->getChristianname().'  ');
            ...
            if (!empty($bangDiem)) {
                ...
                $sWriter->alignCurrentCellCenter();
                $sWriter->writeCell($phanBo->getPhanDoan());

                $sWriter->goRight();
                $sWriter->alignCurrentCellCenter();
                $sWriter->writeCellAndGoRight($phanBo->getChiDoan()->getName());
    ...
    public function writeBangDiemDauNam()
    {
        $this->createSpreadsheetWriter();
        $this->writeHeadingDauNam($this->huynhTruong->getPhanBo()->getNamHoc()->getId(), null, null);
        $this->writeDataDauNam($this->getPhanBoThieuNhi());
    }
